<template>
  <div>
    ...
    <v-container fluid grid-list-md style="margin-top:10px">
        <h1 style="text-align:center">실시간 드론 중계 현황</h1>
        <v-layout row wrap style="margin-top:10px">
            <v-flex xs12 md8 class="row fill-height">
             <v-card>
            ...
            <v-flex xs12 md4>
                <v-card v-for="(drone,index) in drones" :key="index">
                    <v-card-title primary-title class="justify-center">
                        Drone - {{index+1}} 화면
                    </v-card-title>
                    <v-divider></v-divider>
                    <v-card-text>
                        드론 {{index+1}}} 화면 보여줘요~
                        <v-img :src="getImgUrl(drone.src)" contian height="100px">
                        </v-img>
                    </v-card-text>
                </v-card>
            </v-flex>
